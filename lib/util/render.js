...
function forbidden(req, res) {
  ...
  page(req, res, 'armchairdj/page/error/forbidden');
}
...
function notFound(req, res) {
  ...
  page(req, res, 'armchairdj/page/error/notFound');
}
...
function serverError(req, res, err) {
  ...
  page(req, res, 'armchairdj/page/error/serverError');
}
...
function mergeKeywords(keywords) {
  return _.chain([i18n('armchairdj.meta.keywords'), keywords]).flatten().compact().sort().value();
}
