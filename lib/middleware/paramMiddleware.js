...
function playlistIdParam(req, res, next, playlistId) {
  ...
  Playlist.findById(playlistId, handlePlaylist);
...
function postIdParam(req, res, next, postId) {
  ...
  Post.findById(postId).populate('author playlist release tags').exec(handlePost);
...
function releaseIdParam(req, res, next, releaseId) {
  ...
  Release.findById(releaseId, handleRelease);
